...
/* =====================================================
   ...
   State kamera/AI dideklarasikan sebelum fungsi inti.
   Elemen kamera aktual aplikasi: #kamera dan #aiCanvas.
===================================================== */
/* =====================================================
   KONSTANTA
===================================================== */
...
let tanggalAbsenSesi = null;

function resetStatusAbsensiHarianJikaBerubah() {
    try {
        const tanggalHariIni = tanggalISOBaru(new Date(dapatkanWaktuAplikasi()));
        if (tanggalAbsenSesi && tanggalAbsenSesi !== tanggalHariIni) {
            sudahAbsen = false;
            tanggalAbsenSesi = null;
        }
    } catch (_) {}
}
...
// Synthetic face tracker used only when Human does not provide face.id.
// It matches consecutive detections by nearest face-center/size instead of
// a coarse 32px grid so small movements do not reset liveness state.
let syntheticFaceTracks = new Map();
let faceObjectTrackKeys = new WeakMap();
let nextSyntheticFaceTrackId = 1;
const SYNTHETIC_FACE_TRACK_MAX_AGE_MS = 1200;
const SYNTHETIC_FACE_TRACK_CLAIM_MS = 80;
...
async function turunkanResolusiKamera(stream = video?.srcObject) {
    ...
    try {
        ...
        const isPortrait = konfigurasi?.rasioKamera === '9:16';
        const targetW = isPortrait ? 480 : 640;
        const targetH = isPortrait ? 640 : 480;
        const idealWidth = maxWidth > 0 ? Math.min(targetW, maxWidth) : targetW;
        const idealHeight = maxHeight > 0 ? Math.min(targetH, maxHeight) : targetH;
...
async function statusIzinKameraBrowser() {
    try {
...
function tampilkanErrorKamera(error) {
    ...
    if (teksErrorKamera) {
        const tambahanIframe = window.top !== window.self
            ? '\n\n' + t('cameraIframeError')
            : '';
        const namaError = String(error?.name || 'UnknownError');
        const detailError = String(error?.message || '').trim();
        const detailTeknis = detailError
            ? `\n\nKode teknis: ${namaError}\nDetail: ${detailError}`
            : `\n\nKode teknis: ${namaError}`;
    ...
    // Tidak semua browser mengimplementasikan Permissions API untuk kamera.
    // Jika tersedia, simpan statusnya hanya sebagai diagnostik agar support
    // dapat membedakan blocked/denied dari kegagalan hardware.
    void statusIzinKameraBrowser().then(state => {
        if (!state || !teksErrorKamera) return;
        const suffix = `\nStatus izin browser: ${state}`;
        if (!teksErrorKamera.textContent.includes('Status izin browser:')) {
            teksErrorKamera.textContent += suffix;
        }
    });
    ...
    try {
        ...
        const records = await new Promise((resolve, reject) => {
            try {
                ...
                tx.onerror = () => reject(tx.error || new Error('Gagal membaca transaksi riwayat absensi.'));
                tx.onabort = () => reject(tx.error || new Error('Pembacaan riwayat absensi dibatalkan.'));
            } catch (error) {
                reject(error);
            }
        });
        ...
        if (!kedaluwarsa.length) return 0;
...
async function tampilkanKamera() {
    resetStatusAbsensiHarianJikaBerubah();
...
function setLocalStorageSafe(key, value, { preserveKeys = [] } = {}) {
    try {
        ...
        if (isQuotaExceededError(error)) {
            // preserveKeys berarti kunci yang wajib dipertahankan, bukan dihapus.
            // Jangan pernah menghapus PIN/lock state hanya untuk membebaskan quota.
            const protectedKeys = new Set([
                String(key),
                ...preserveKeys.filter(Boolean).map(String)
            ]);

            // Hanya bersihkan cache lama yang memang aman untuk dibuang.
            const cleanupKeys = [
                'absensi_piket_saran_v1'
            ];
            for (const removable of cleanupKeys) {
                if (protectedKeys.has(removable)) continue;
...
const humanConfig = {
    ...
    face: {
        ...
        // Human 3.3.6 exposes detector/mesh/liveness/etc. but no face.tracker config key;
        // stable IDs are handled by kunciTrackerWajah() below.
        detector: {
            enabled: true,
            rotation: true,
            return: true,
...
function sinkronkanCanvasAIPadaResize() {
    ...
    const clientWidth = Math.max(1, Math.round(video.clientWidth || width));
    const clientHeight = Math.max(1, Math.round(video.clientHeight || height));
...
async function mulaiKameraInternal() {
    ...
    try {
        ...
        const facingModeDiminta = gunakanKameraDepan ? 'user' : 'environment';
            // Jangan menggunakan facingMode: exact + aspectRatio + max width/height
        // pada request pertama. Kombinasi tersebut terlalu ketat pada sebagian
        // Android/Brave/Safari dan dapat menghasilkan OverconstrainedError atau
        // membuat driver kamera gagal memilih sensor yang valid. Ideal constraint
        // tetap mengarahkan kamera depan/rasio yang diinginkan tanpa memaksa
        // perangkat memiliki resolusi persis tertentu.
        ...
        const constraintCandidates = [
            constraintsUtama,
            {
                ...
                video: {
                    facingMode: { ideal: facingModeDiminta },
                    width: { ideal: rasio === '9:16' ? 480 : 640 },
                    height: { ideal: rasio === '9:16' ? 640 : 480 },
                    frameRate: { ideal: 24 }
                }
            },
            { audio: false, video: { facingMode: { ideal: facingModeDiminta } } },
            { audio: false, video: true }
        ];

        let errorKameraTerakhir = null;
        for (let i = 0; i < constraintCandidates.length; i++) {
            try {
                stream = await denganBatasWaktuKamera(
                    navigator.mediaDevices.getUserMedia(constraintCandidates[i]),
                    KAMERA_TIMEOUT_MS,
                    pesanTimeoutKamera
                );
                if (stream) break;
            } catch (errorKandidat) {
                errorKameraTerakhir = errorKandidat;
                // Jangan mencoba ulang bila izin ditolak, perangkat kamera sedang
                // dipakai aplikasi lain, atau konteks browser tidak aman. Retry
                // hanya berguna untuk constraint yang tidak didukung perangkat.
                const retryable = ['OverconstrainedError', 'ConstraintNotSatisfiedError'].includes(errorKandidat?.name);
                if (!retryable) throw errorKandidat;
            }
        }

        if (!stream) throw errorKameraTerakhir || new Error(pesanTimeoutKamera);
        ...
        // Pastikan atribut media mobile benar meskipun HTML dimodifikasi/
        // di-cache oleh browser lama. muted + playsInline mencegah Safari/iOS
        // memindahkan preview ke fullscreen atau menolak autoplay.
        video.autoplay = true;
        video.muted = true;
        video.playsInline = true;
        video.setAttribute('playsinline', '');
...
function hentikanKamera(invalidatePendingRequests = true) {
    ...
    if (stream) matikanKamera(stream);
...
function normalisasiKotakTrackerWajah(face) {
    const box = Array.isArray(face?.box) ? face.box : [0, 0, 0, 0];
    const x = Number(box[0]) || 0;
    const y = Number(box[1]) || 0;
    const w = Math.max(1, Number(box[2]) || 1);
    const h = Math.max(1, Number(box[3]) || 1);
    return {
        x, y, w, h,
        cx: x + w / 2,
        cy: y + h / 2,
...
function kunciTrackerWajah(face, index = 0) {
    ...
    if (face && typeof face === 'object' && faceObjectTrackKeys.has(face)) {
        return faceObjectTrackKeys.get(face);
    }

    const box = normalisasiKotakTrackerWajah(face);
    const now = performance.now();

    // Expire old synthetic tracks to prevent unbounded growth.
    for (const [key, track] of syntheticFaceTracks) {
        if (now - track.lastSeen > SYNTHETIC_FACE_TRACK_MAX_AGE_MS) {
            syntheticFaceTracks.delete(key);
        }
    }

    let bestKey = null;
    let bestScore = Number.POSITIVE_INFINITY;
    const matchThreshold = Math.max(90, box.size * 0.9);

    for (const [key, track] of syntheticFaceTracks) {
        const recentlyClaimedByOtherFace =
            track.lastFaceObject && track.lastFaceObject !== face &&
            now - track.lastClaimAt < SYNTHETIC_FACE_TRACK_CLAIM_MS;
        if (recentlyClaimedByOtherFace) continue;

        const distance = Math.hypot(box.cx - track.cx, box.cy - track.cy);
        const sizeDelta = Math.abs(box.size - track.size);
        const score = distance + sizeDelta * 0.35;
        if (score < bestScore) {
            bestScore = score;
            bestKey = key;
        }
    ...
    if (!bestKey || bestScore > matchThreshold) {
        bestKey = `track:${nextSyntheticFaceTrackId++}`;
    }

    syntheticFaceTracks.set(bestKey, {
        ...box,
        lastSeen: now,
        lastClaimAt: now,
        lastFaceObject: face || null,
        index
    });

    if (face && typeof face === 'object') {
        faceObjectTrackKeys.set(face, bestKey);
    }
...
function resetTrackerLiveness() {
    ...
    syntheticFaceTracks.clear();
    faceObjectTrackKeys = new WeakMap();
    nextSyntheticFaceTrackId = 1;
}
...
document.getElementById(
    ...
    async () => {
        ...
        try {
            ...
            sudahAbsen = true;
            try { tanggalAbsenSesi = tanggalISOBaru(new Date(dapatkanWaktuAplikasi())); } catch (_) { tanggalAbsenSesi = tanggalISOBaru(new Date()); }
            hentikanKamera();
...
async function resetSemuaData() {
    ...
    try {
        ...
        localStorage.removeItem(GURU_PIN_LOCK_KEY);
        localStorage.removeItem(GURU_PIN_FAIL_KEY);
    } catch (error) {
...
function tampilkanPreviewFotoSaran(peran, blob) {
    ...
    // Pertahankan object URL sampai preview dibersihkan oleh resetInputFotoSaran()
    // atau tutupKotakSaran(). Ini menghindari blank preview pada sebagian Safari/iOS.
...
document.addEventListener('keydown', event => {
    ...
    document.querySelectorAll(
        '.modal.muncul, .manual-override-modal.muncul, .guru-pin-modal.muncul, ' +
        '#inAppBrowserWarning:not([hidden]), #appUpdateModal:not([hidden]), #iframePermissionModal:not([hidden])'
    ).forEach(modal => {
...
async function hapusSemuaRiwayatAbsensiDB() {
    ...
    return new Promise((resolve, reject) => {
        ...
        try {
            const stores = ['attendanceRecords', 'attendancePhotos'];
            if (db.objectStoreNames.contains(ATTENDANCE_OUTBOX_STORE)) {
                stores.push(ATTENDANCE_OUTBOX_STORE);
            }
            const tx = db.transaction(stores, 'readwrite');
            tx.objectStore('attendanceRecords').clear();
            tx.objectStore('attendancePhotos').clear();
            if (stores.includes(ATTENDANCE_OUTBOX_STORE)) {
                tx.objectStore(ATTENDANCE_OUTBOX_STORE).clear();
            }
            tx.oncomplete = () => {
                db.close();
                finish(resolve);
...
/* =====================================================
   PREFERENSI EVENT
===================================================== */
function pasangPreferensiEvent() {
...
function tampilkanPeringatanInAppBrowser() {
    ...
    document.getElementById('btnBukaBrowser')?.addEventListener('click', async () => {
        ...
        // window.open harus dipanggil saat user activation masih aktif.
        let popup = null;
...
onDomReady(pasangPreferensiEvent);
...
onDomReady(pasangControlEvents);
